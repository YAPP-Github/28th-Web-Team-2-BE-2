...
@Component
public class GsShopProductDetailParser {
    ...
    private static final Pattern DELIVERY_NOTE_PATTERN = Pattern.compile("\\\"prdDtlDlvInfo\\\":\\\"([^\\\"]+)\\\"");

    public BigDecimal parsePricePer100g(final String html) {
        if (html == null || html.isBlank()) {
            return null;
        }
        return Jsoup.parse(html).getElementsContainingOwnText("당").stream()
                .map(Element::text)
    ...
    public BigDecimal parsePricePer100g(
        ...
        final Matcher matcher = PRODUCT_UNIT_PATTERN.matcher(productNameOrEmpty(productName));
        if (!matcher.find()) {
            return null;
        }
    ...
    public String parseDeliveryNote(final String html) {
        ...
        final Matcher matcher = DELIVERY_NOTE_PATTERN.matcher(html);
        if (matcher.find() && !matcher.group(1).isBlank()) {
            return matcher.group(1).trim();
        }
        final Element delivery = Jsoup.parse(html).selectFirst(".delivery, [class*='delivery']");
    ...
    private String productNameOrEmpty(final String productName) {
        if (productName == null) {
            return "";
        }
        return productName;
