...
public class StoreResultConverter {
    ...
    public RecommendedStoresResponse toRecommendedStoresResponse(final RecommendedStoresResult result) {
        return new RecommendedStoresResponse(result.totalCount(), result.stores().stream()
                .map(this::toRecommendedStoreResponse).toList());
    }

    private RecommendedStoreResponse toRecommendedStoreResponse(final RecommendedStoreResult result) {
        return new RecommendedStoreResponse(result.storeId(), result.storeName(), result.latitude(),
                result.longitude(), result.addressName(), result.roadAddressName(), result.phone(),
                result.placeUrl(), result.distanceMeters(), result.cheapItemCount(), result.itemNames(),
