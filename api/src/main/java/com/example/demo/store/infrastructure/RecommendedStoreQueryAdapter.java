...
public class RecommendedStoreQueryAdapter implements RecommendedStoreQueryPort {
    ...
    @Override
    public List<RecommendedStoreSource> findLatestCheapReports(final Long itemId) {
        final List<UserReport> reports = userReportJpaRepository.findLatestCheapReportsByItemId(itemId);
        final Map<Long, Store> stores = findStores(reports);
        return reports.stream()
                .map(report -> toSource(report, stores.get(report.storeId())))
    ...
    private RecommendedStoreSource toSource(final UserReport report, final Store store) {
