...
public class KakaoRegionSearchAdapter implements RegionSearchQueryPort {
    ...
    public RegionSearchResult search(final RegionSearchQuery query) {
        ...
        result.addresses().forEach(address -> addRegion(regions, address));
        return new RegionSearchResult(regions.values().stream().toList());
    }

    private void addRegion(final Map<String, RegionSearchResult.Region> regions, final KakaoAddress kakaoAddress) {
        final Address address = kakaoAddress.address();
        ...
        regions.putIfAbsent(bCode, new RegionSearchResult.Region(
                ...
                kakaoAddress.latitude(),
                kakaoAddress.longitude()));
    }
}
