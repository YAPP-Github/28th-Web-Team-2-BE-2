...
class RegionItemReportE2ETest {
    ...
    private Long firstPriceTieReportId;
    ...
    void setUp() {
        ...
        storeId = storeJpaRepository.save(new Store(
                ...
                .id();
        firstPriceTieReportId = save(
                        potato.id(), storeId, 3000, "1kg", -500, new BigDecimal("-14.29"))
                .id();
    ...
    @Test
    @DisplayName("같은 가격이면 reportId 오름차순으로 안정 정렬한다")
    void sortsSamePriceByReportIdAscending() throws Exception {
        final Long secondPriceTieReportId = save(
                        potatoId, storeId, 3000, "1kg", -500, new BigDecimal("-14.29"))
                .id();

        mockMvc.perform(get(path(REGION_ID, potatoId)).param("sort", "PRICE_ASC"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.reports[1].reportId").value(firstPriceTieReportId))
                .andExpect(jsonPath("$.data.reports[2].reportId").value(secondPriceTieReportId));
    }
    ...
    void defaultsToLatestSort() throws Exception {
        ...
        mockMvc.perform(get(path(REGION_ID, potatoId)))
                ...
                .andExpect(jsonPath("$.data.reports[0].reportedAt").value(today.toString()))
                .andExpect(jsonPath("$.data.reports[3].reportedAt")
                        .value(today.minusDays(3).toString()));
    }

    @Test
    @DisplayName("같은 제보 기준일이면 reportId 최신순으로 안정 정렬한다")
    void sortsSameReportDateByReportIdDescending() throws Exception {
        final LocalDate yesterday = LocalDate.now(ZoneId.of("Asia/Seoul")).minusDays(1);
        jdbcTemplate.update("UPDATE user_reports SET report_date = ?", yesterday);
        final Long firstReportId = save(
                        potatoId, storeId, 5_000, "1kg", -500, new BigDecimal("-9.09"))
                .id();
        final Long secondReportId = save(
                        potatoId, storeId, 5_500, "1kg", -500, new BigDecimal("-8.33"))
                .id();

        mockMvc.perform(get(path(REGION_ID, potatoId)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.reports[0].reportId").value(secondReportId))
                .andExpect(jsonPath("$.data.reports[1].reportId").value(firstReportId));
    }
    ...
    void rejectsUnknownRegion() throws Exception {
        mockMvc.perform(get(path(OTHER_REGION_ID, potatoId))).andExpect(status().isNotFound());
    }
