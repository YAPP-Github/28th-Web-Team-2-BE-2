...
class DemoApplicationTests {
    ...
    void swagger_ui와_openapi_문서를_설정된_메타데이터와_함께_제공한다() throws Exception {
        ...
        mockMvc.perform(get("/v3/api-docs"))
                ...
                .andExpect(jsonPath("$.paths['/api/v1/regions/nearby'].get.description")
                        .value("응답의 latitude와 longitude는 지역 대표 좌표가 아니라 조회 요청 좌표다."))
                .andExpect(jsonPath("$.components.schemas.NearbyRegionResponse.properties.latitude")
                        .exists())
                .andExpect(jsonPath("$.components.schemas.NearbyRegionResponse.properties.longitude")
                        .exists())
